...
# client = MongoClient('mongodb://이름:이름@localhost', 27017)
db = client.week1_projectdb
...
@app.route('/update_foodloc', methods=['POST'])
def save_food():
    token_receive = request.cookies.get('mytoken')
    try:
        payload = jwt.decode(token_receive, SECRET_KEY, algorithms=['HS256'])
        ...
        new_doc = {
            ...
            "food_name": name_receive,
            "food_loc": loc_receive,
            "food_comment": about_receive,
            "food_file": f'{filename}.{extension}',
            "date": today_receive
        }

        db.foodloc.insert_one(new_doc)

        return jsonify({"result": "success", 'msg': '음식점을 등록했습니다.'})
    except (jwt.ExpiredSignatureError, jwt.exceptions.DecodeError):
        return redirect(url_for("home"))
